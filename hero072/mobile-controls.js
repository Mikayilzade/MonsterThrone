(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.MonsterThroneMobile=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function sanctuaryRetreatMove(entity,angle,distance,isValid){const turns=[0,Math.PI/3,-Math.PI/3,Math.PI/2,-Math.PI/2,Math.PI];for(const turn of turns){const direction=angle+turn,x=entity.x+Math.cos(direction)*distance,y=entity.y+Math.sin(direction)*distance;if(isValid(x,y)){entity.x=x;entity.y=y;entity.dir=direction;return {moved:true,angle:direction};}}return {moved:false,angle};}
