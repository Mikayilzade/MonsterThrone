(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.MonsterThroneMobile=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function minimapLayout(width,height,landscape=false,touchLayout=width<=900){const compact=touchLayout&&landscape&&height<=430,widthLimit=compact&&width<=950,r=touchLayout?(widthLimit?36:landscape?42:46):72,bottom=touchLayout?(widthLimit?104:landscape?132:286):16;return {x:width-r-12,y:Math.max(r+12,height-r-bottom),r};}
  function compactLandscapeLayout(width,height){const compact=height<=430&&width>height&&width<=950;return {compact,hotbarSlots:compact?4:8,joystickSize:compact?88:112,actionWidth:compact?54:64,minTouchHeight:compact?44:54,minimapRadius:compact?36:height<width?42:46};}
  ...
  function clearSanctuaryAvoidance(entity){entity.unfairDamage=false;entity.sanctuaryAvoidUntil=0;entity.sanctuaryAvoidMaxUntil=0;}
  function markSanctuaryAvoidance(entity,worldTime,minTime=4,maxTime=12){entity.unfairDamage=true;entity.sanctuaryAvoidUntil=worldTime+minTime;entity.sanctuaryAvoidMaxUntil=worldTime+Math.max(minTime,maxTime);}
  function retreatState(entity,hero,camp,radius,aggro=140,worldTime=0){if(!entity?.unfairDamage)return null;const retreatDistance=Math.max(140,aggro+60),boundaryDistance=Math.hypot(entity.x-camp.x,entity.y-camp.y)-radius,minTimeDone=worldTime>=(entity.sanctuaryAvoidUntil||0),timedOut=worldTime>=(entity.sanctuaryAvoidMaxUntil||Infinity);if(timedOut||(minTimeDone&&boundaryDistance>=retreatDistance)){clearSanctuaryAvoidance(entity);return {state:'resume'};}return {state:'sanctuary-retreat',angle:Math.atan2(entity.y-camp.y,entity.x-camp.x),retreatDistance};}
  function sanctuaryRetreatMove(entity,camp,radius,angle,distance,isLand){const startDistance=Math.hypot(entity.x-camp.x,entity.y-camp.y),turns=[0,Math.PI/3,-Math.PI/3,Math.PI/2,-Math.PI/2];for(const turn of turns){const direction=angle+turn,x=entity.x+Math.cos(direction)*distance,y=entity.y+Math.sin(direction)*distance,campDistance=Math.hypot(x-camp.x,y-camp.y);if(campDistance>=radius&&campDistance+1e-9>=startDistance&&isLand(x,y)){entity.x=x;entity.y=y;entity.dir=direction;return {moved:true,angle:direction};}}return {moved:false,angle};}
  ...
  return {bindPointerButtons,normalizedDirection,forwardTarget,inSafeZone,ejectFromSafeZone,applySafeZoneBoundary,canDamageHero,canAttackTarget,canPlaceTrap,controlsTabVisible,createPinchState,minimapLayout,compactLandscapeLayout,cameraZoomValue,targetAfterWorldTap,zoomShortcutCommand,pointerAimAfterZoom,cameraViewport,worldToScreen,validTarget,targetTransition,markSanctuaryAvoidance,retreatState,sanctuaryRetreatMove,nearestLootableCarcass,shouldRefreshInventory,corpsePileLayout,hotbarSignature,clamp};
});
